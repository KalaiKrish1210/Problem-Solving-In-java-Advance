		...
		for (int a = 0; a < arr.length; a++) {
			...
			for (int b = a + 1; b < arr.length; b++) {
				...
				if (arr[a] == arr[b]) {
					...
					// getting increased count greater than 0 and assign the arr[b] = o to avoid the
			...
			// Comparing if the count is equal to the zero and arr[a] not equal to 0
			if (count == 0 && arr[a] != 0) {
				System.out.println("Remaining  Values\t" + arr[a]);
				// Print the duplicate value
			}
		}
